...
const boxCls = 'rounded border border-blue-300 bg-blue-50/40 px-3 py-2';
...
const kumarMukesh = {
    ...
    renderHeader: (r, set) => (
        <div className="text-center">
            <Field value={r.name} onChange={(v) => set('name', v)} ph="YOUR NAME" className="block text-[2.7em] font-bold leading-tight" />
            <div className="mt-1.5 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[0.92em]">
            ...
            <Field value={r.location} onChange={(v) => set('location', v)} ph="Location" className="mt-1 block text-[0.95em]" />
        </div>
    ),

    renderTitle: (title) => (
        <>
            <h2 className="text-[1.3em] font-bold text-[color:var(--rb-accent)]">{title}</h2>
            <div className="mb-2 mt-0.5 border-t border-[color:var(--rb-accent)]" />
        </>
    ),
    ...
    renderItem: (kind, ctx) => {
                ...
                <div className={boxCls}>
                    <div className="flex items-baseline justify-between gap-4">
                        <p className="min-w-0 flex-1">
                            ...
                            <Field value={item.location} onChange={(v) => update({ location: v })} ph="Location" className="font-bold" />
                        </p>
                        <PeriodField value={item.period} onChange={(v) => update({ period: v })} className={periodCls} />
                    </div>
                    <Field value={item.secondary} onChange={(v) => update({ secondary: v })} ph={secondaryPh} className="block italic" />
        ...
        if (kind === 'edu') {
            return (
                <div className={boxCls}>
                    ...
                    <Field value={item.degree} onChange={(v) => update({ degree: v })} ph="Degree and field of study" className="block" />
                    <p className="italic">
                        <span>Location: </span>
        ...
        if (kind === 'proj') {
            return (
                <>
                    <div className="flex items-baseline justify-between gap-4">
                        <Field value={item.primary} onChange={(v) => update({ primary: v })} ph={primaryPh} className="min-w-0 flex-1 font-bold" />
                    ...
                    <Field value={item.secondary} onChange={(v) => update({ secondary: v })} ph={secondaryPh} className="block italic text-slate-700" />
                    <Bullets bullets={bullets} />
                </>
            );
        }
