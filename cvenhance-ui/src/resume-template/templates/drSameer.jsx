...
const drSameer = {
    ...
    renderHeader: (r, set) => (
        <div className="text-center">
            ...
            <div className="mt-1 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[0.92em]">
            ...
            <div className="mt-2 border-t border-slate-900" />
        </div>
    ),

    renderTitle: (title) => (
        <h2 className="mb-1.5 mt-1 text-[1.2em] font-bold">{title}</h2>
    ),
    ...
    renderItem: (kind, ctx) => {
        ...
        if (kind === 'edu') {
            return (
                <>
                    <Field value={item.school} onChange={(v) => update({ school: v })} ph="School / University" className="block font-bold" />
                    <div className="flex items-baseline justify-between gap-4">
