...
const ICONS = {
    location: 'M12 2a7 7 0 00-7 7c0 5 7 13 7 13s7-8 7-13a7 7 0 00-7-7zm0 9.5A2.5 2.5 0 1112 6.5a2.5 2.5 0 010 5z',
...
const mrAbrahm = {
    ...
    renderHeader: (r, set) => (
        <div className="text-left">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[0.92em]">
                <Contact icon="location" value={r.location} onChange={(v) => set('location', v)} ph="Location" />
            ...
            <div className="mt-2 border-t-2 border-slate-800" />
        </div>
    ),

    renderTitle: (title) => (
        <h2 className="mb-1.5 text-[1.3em] font-bold">{title}</h2>
    ),
        ...
        if (kind === 'edu') {
            return (
                <>
                    ...
                    <div className="flex items-baseline justify-between gap-4">
                        <Field value={item.degree} onChange={(v) => update({ degree: v })} ph="Degree and field of study" />
                        <Field value={item.location} onChange={(v) => update({ location: v })} ph="Location" className="shrink-0" />
                    </div>
                </>
            );
